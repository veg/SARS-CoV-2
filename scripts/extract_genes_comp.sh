...
then 
   echo "$GENE alignment already processed"
    ...
    then
        ...
        #mpirun -np $NP --use-hwthread-cpus $HYPHYMPI  
        $HYPHY /Users/sergei/Development/hyphy-analyses/codon-msa/pre-msa.bf --input $FILE --reference $REFERENCE_SEQUENCE --E 0.000001 --trim-from $TRIM_FROM --trim-to $TRIM_TO --N-fraction $N_FRAC --keep-reference Yes
