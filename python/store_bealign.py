...
def store_bealign_file(input, gene):
    ...
    for seq in seqs:
        ...
        items_to_update[seq.id][gene + '_bealign_seq'] = seq


    to_update = [update_record(gene + '_bealign_seq', v[gene + '_bealign_seq'], gene) for k,v in items_to_update.items()]
...
if __name__ == "__main__":
    ...
    arguments.add_argument('-i', '--bealign-input',   help = 'fasta to update', required = True, type = str)
    arguments.add_argument('-t', '--type',   help = 'gene region', required = True, type = str)
    args = arguments.parse_args()
    store_bealign(args.input, args.type)
